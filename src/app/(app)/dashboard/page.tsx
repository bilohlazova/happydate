...
export default function CalendarPage() {
  ...
  useEffect(() => {
    ...
    void fetch(`/api/calendar/observances?v=3&year=${viewYear}&locale=${encodeURIComponent(locale)}&country=${encodeURIComponent(calendarCountry)}`, { signal: controller.signal })
      .then(async (response) => {
        if (!response.ok) throw new Error("Holiday calendar unavailable");
        return response.json() as Promise<{
          country: LocalObservance["country"];
          countries: Record<string, string>;
          observances: Array<{ date: string; title: string; kind: LocalObservance["kind"] }>;
        }>;
      })
