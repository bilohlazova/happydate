...
export default function PeoplePage() {
  ...
  // ── useCallback щоб useEffect не скаржився ──
  ...
  return (
    <>
      <style>{`
        ...
        .pp-card { background:#fff; border-radius:18px; border:1.5px solid #ede9f8; padding:14px 16px; display:flex; align-items:center; gap:14px; cursor:pointer; box-shadow:0 1px 4px rgba(0,0,0,.04); transition:transform .15s,border-color .15s; -webkit-tap-highlight-color:transparent; position:relative; }
        ...
        .pp-icon-btn { width:34px; height:34px; border-radius:10px; border:none; display:flex; align-items:center; justify-content:center; cursor:pointer; font-size:15px; transition:all .15s; flex-shrink:0; }
        ...
        .pp-delete-confirm { position:absolute; right:0; top:calc(100% + 6px); background:#fff; border:1.5px solid #fde8e8; border-radius:14px; padding:12px 14px; box-shadow:0 8px 24px rgba(0,0,0,.12); z-index:10; min-width:200px; animation:ppPop .2s cubic-bezier(.34,1.3,.64,1); }
      ...
      <div className="pp-root" onClick={() => setShowDeleteFor(null)}>
        <div className="pp-header">
          <div className="pp-header-top">
            ...
            <button className="pp-add-btn" onClick={openAdd}><span>＋</span> Dodaj</button>
          </div>
        </div>

        <div className="pp-search">
          <span style={{ fontSize:16, color:"#b0a8cc" }}>🔍</span>
          <input placeholder="Szukaj osoby..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
              ...
              <div key={person.id} className="pp-card-wrap">
                <div className="pp-card" onClick={() => router.push(`/person/${person.id}`)}>
                  ...
                  <div className="pp-card-actions" onClick={e => e.stopPropagation()}>
                    <button className="pp-icon-btn pp-icon-btn-edit" onClick={e => openEdit(person, e)} title="Edytuj">✏️</button>
                    <button className="pp-icon-btn pp-icon-btn-delete" onClick={e => { e.stopPropagation(); setShowDeleteFor(isDeleteOpen ? null : person.id); }} title="Usuń">🗑️</button>
                  </div>
                </div>
                ...
                {isDeleteOpen && (
                  <div className="pp-delete-confirm" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="pp-delete-confirm-btns">
                      ...
                      <button className="pp-delete-yes" disabled={deleting === person.id} onClick={e => deletePerson(person.id, e)}>
      ...
      {showModal && (
        <div className="pp-overlay" onClick={e => { if (e.target===e.currentTarget) closeModal(); }}>
          <div className="pp-modal">
            ...
            <div className="pp-modal-title">{modalMode === "add" ? "Nowa osoba ✨" : "Edytuj osobę ✏️"}</div>

            <div className="pp-field">
              <label className="pp-label">Imię i nazwisko</label>
              <input className="pp-input" placeholder="np. Anna Kowalska" value={formName} onChange={e => setFormName(e.target.value)} autoFocus />
            </div>
            <div className="pp-field">
              <label className="pp-label">Data urodzin (opcjonalnie)</label>
              <input className="pp-input" type="date" value={formBirthday} onChange={e => setFormBirthday(e.target.value)} />
            </div>
            <div className="pp-field">
